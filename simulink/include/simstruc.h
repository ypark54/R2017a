...
struct _ssStates {
    ...
    struct {
        ...
        unsigned int funcIsRowMajor:     1;  /* Specify row or column function*/
...
#define ssGetFuncRowMajor(S) \
            ((S)->states.flags.funcIsRowMajor)
#define ssSetFuncRowMajor(S, val) \
{           if(ssRTWGenIsCodeGen(S) && !(ssIsRapidAcceleratorActive(S))) \
            { \
                ((S)->states.flags.funcIsRowMajor = val); \
            } \
}

 /* Get/Set for Model Param 'RowMajorDimensionSupport */
#define ssGetIsModelRowMajor(S) \
