...
mObject; size_t mM; size_t mN; const char * mUnit; real_T mNominalValue; const
char * mNominalUnit; NeNominalSource mNominalSource; NeInitMode mInitMode;
...
NeDaeTag NeDae; typedef struct NeProfilerTag NeProfiler; typedef struct
NeCustomDataSizeTag NeCustomDataSize; typedef struct NeCustomDataTag
NeCustomData; typedef struct NeRealFunctorDataTag NeRealFunctorData; typedef
struct NeRealFunctorTag NeRealFunctor; typedef struct SlSparseMatrixTag {
PmSparsityPattern *mPattern; PmRealVector *mPr; } SlSparseMatrix; typedef struct
SLMatricesTag { boolean_T mIsYAffineTermUpdated; boolean_T
...
*mOutputFunctionMap; size_t mNumIo[NUM_IO_TYPES]; const NeDsIoInfo
*mIo[NUM_IO_TYPES]; NeChecksum *mChecksum; NeProfiler *mProfiler;
NeDaePrivateData *mPrivateData; void (*mDestroy) (NeDae *); NeDae* (*mCloneFcn)
(const NeDae *); NeDaeMethod mMethods[NE_NUM_DAE_METHODS]; NeDaeSolve
mSolves[NE_DAE_SOLVE_NUM]; NeDaeSolve mCheck; NeDaeSetParameters mSetParameters;
NeDaeFunction mPreTrDiagnoseFcn; NeDaeFunction mPreDcDiagnoseFcn; NeDaeSolve
mAllInputsWorkFcn; NeDaeLinearizeFcn mResidualsFcn; NeDaeLinearizeFcn
mLinearizeFcn; PmfMessageId (*mInitializeFcn) (NeDae *dae, NeModelParameters mp,
NeuDiagnosticManager *mgr); void (*mSolverFailureNotify) (NeDae *dae, size_t
num_consecutive_failures); void (*mSetupLoggerFcn) (const NeDae *,
NeLoggerBuilder *); int32_T (*mLogFcn) (const NeDae *, const NeSystemInput *,
PmRealVector *); void (*mGenerateFcn) (const NeDae *, const NeCgParams *,
NeCgResults *); const SLMatrices ** (*mSwitchedLinearDataFcn) (const NeDae *);
NeDaeMakeOutputFcn mMakeOutput[NE_NUM_DAE_METHODS]; NeDaeFreeOutputFcn
